...
document.addEventListener('keydown', (event) => {
  if (
    event.key === 'ArrowUp' &&
    !matrix.isNextMoveInContactWithBlocksOrBottom(1)
  ) {
    matrix.rotate()
    render()
  } else if (
    event.key === 'ArrowLeft' &&
    !matrix.isNextMoveInContactWithBlocksOrBottom(4)
  ) {
    matrix.goLeft()
    render()
  } else if (
    event.key === 'ArrowRight' &&
    !matrix.isNextMoveInContactWithBlocksOrBottom(2)
  ) {
    matrix.goRight()
    render()
  } else if (
    event.key === 'ArrowDown' &&
    !matrix.isNextMoveInContactWithBlocksOrBottom(3)
  ) {
    matrix.goDown()
    render()
  }
  ...
  base = base
    .map((line, y) => {
      return line.map((unit, x) => {
        return y < pieceToPrint[0].length &&
          x < pieceToPrint.length &&
          pieceToPrint[y][x] === 1
          ? color
          : 0
      })
...
const enterNewScoreInLocalStorage = (name, score) => {
  const newLeaderBoard = JSON.parse(localStorage.getItem('leaderBoard')).filter(
    (line, i) => i < 9
  )
  ...
  localStorage.setItem(
    'leaderBoard',
    JSON.stringify(newLeaderBoard.sort((a, b) => b.score - a.score))
  )
}

const enterNewScore = (ulLeaderBoard, wholeLeaderBoard) =>{
  ...
  if (matrix.score > scores[scores.length - 1].score) {
    ...
    newLeaderBoard
      ...
      .forEach((player) => {
        ...
        playerLi.innerHTML =
          player.name === 'temp'
            ? `<input id="newScore"> <div>${player.score}</div>`
    ...
    document.querySelector('#enterNewScore').addEventListener('click', () => {
      enterNewScore(ulLeaderBoard,wholeLeaderBoard)
    })
    ...
    document.querySelector('#newScore').focus()
      ...
      if (madeFullLinesColored) {
        ...
        setTimeout(() => {
          ...
          if (oldLevel != matrix.level) {
            tetrisMusic.playbackRate =
              matrix.level <= 1 ? 1 : 1 + 0.06 * matrix.level
          }
        }, 400)
