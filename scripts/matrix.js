class Matrix {
  ...
  set Matrix(matrixToSet) {
    if (
      matrixToSet.length === 20 &&
      matrixToSet.every((line) => line.length === 10) &&
      matrixToSet.flat().every((unit) => typeof unit === 'object')
    ) {
      this.matrix = matrixToSet
    }
  }
  increaseLevelIfPossible() {
    const correspondingLevel = this.levelsAndLines.findIndex(
      (val) => this.linesErasedNumber < val
    )
  ...
  isRotationPossibleAgainstTheSideWalls() {
    const nextPositionDistanceToTheLeft = this.queue[0].distanceToTheLeft(
      (this.queue[0].positionIndex + 1) % 4
    )
    const nextPositionDistanceToTheRight = this.queue[0].distanceToTheRight(
      (this.queue[0].positionIndex + 1) % 4
    )
    if (
      this.queue[0].x >= -1 * nextPositionDistanceToTheLeft &&
      this.queue[0].x < 10 - nextPositionDistanceToTheRight &&
      this.queue[0].y + this.queue[0].distanceToTheBottom() < 19
    ) {
      return true
    } else {
      return false
    ...
    const result = this.matrix.map((line, y, arrayM) => {
      return line.map((unit, x, array) => {
        if (
          x >= tetro.x &&
          y >= tetro.y &&
          x < tetro.x + tetro.totalWidth &&
          y < tetro.y + tetro.totalWidth
        ) {
          if (
            unit > 0 &&
            tetro.positions[tetro.positionIndex][y - tetro.y][x - tetro.x] == 1
          ) {
            isProblem = true
          }
          return tetro.positions[tetro.positionIndex][y - tetro.y][
            x - tetro.x
          ] == 1
            ? tetro.color
            : unit
        ...
        if (
          y == arrayM.length - 1 &&
          tetro.y + tetro.distanceToTheBottom() > 19
        ) {
          isProblem = true
        }
        return unit
  ...
  eraseFullLines() {
    ...
    const filtered = this.matrix.filter(
      (line) => !line.every((unit) => unit > 0)
    )
